...
try {
    ...
    $stmt = $conn->query("SELECT id, nom, prenom, categorie, telephone FROM permis where copie_permis != ''");
...
if (isset($_GET['id'])) {
    ...
    if ($stmt->execute()) {
        ...
        header('Location: ?route=liste_permis_international&&success=3');
        exit();
    } else {
        // Redirection avec message d'erreur
        header('Location: ?route=liste_permis_international&&error=1');
        exit();
    }
}
?>
...
<div class="container mt-4">
    <div class="card">
        <div class="card-header d-flex justify-content-between align-items-center">
            ...
            <a href="../service document/formulaire.php?page=permis_international" class="btn btn-primary">Ajouter un Permis</a>
        </div>
                    ...
                    <?php foreach ($permis as $row): ?>
                        <tr>
                            ...
                            <td>
                                ...
                                <a href="?route=liste_permis&&id=<?= urlencode($row['id']) ?>"
                                   class="p-1"
                                        ...
                                        <?php if ($details): ?>
                                            <ul class="list-group">
                                                <?php foreach ($details as $key => $value): ?>
                                                    <?php if ($key === 'photo' || $key === 'copie_pc' || $key === 'copie_permis'): ?>
